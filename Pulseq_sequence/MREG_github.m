...
R = [2.8687 6.0202 2.552 3.0583];
T = stack_of_spirals(R,1,1,0.001*FOV,0.001*Resolution,1); 
trajectStruct_export(T,'2024_Pulsec_SoS_test',1);% g unit T/m to mT/m, normalized
...
adc = mr.makeAdc(adcSamples,'Duration',dW*adcSamples,'system',sys);

mregx = mr.makeArbitraryGrad('x',Grads_calc(1,:),'Delay',adc.delay,'system',sys_true); % looks like the optimized MREG trajectory often exceeds the slew rate so we use an additional system with true maximum limits
mregy = mr.makeArbitraryGrad('y',Grads_calc(2,:),'Delay',adc.delay,'system',sys_true);
mregz = mr.makeArbitraryGrad('z',Grads_calc(3,:),'Delay',adc.delay,'system',sys_true);
...
seq.setDefinition('Name', 'mreg_trial');
%seq.setDefinition('ReceiverGainHigh',1) ;
seq.write('myMREG.seq');   % Output sequence for scanner

  ...
  if (t < numVox)
    {
      ...
      for (int k = 0; k < size; k++)
        {          
	  int j = id[ons+k];
	  tmp1[idx].x +=   (val[j].x * segfil[j]) * we[ons+k].x + 
	    (val[j].y * segfil[j]) * we[ons+k].y;
	  tmp1[idx].y +=   - (val[j].x * segfil[j]) * we[ons+k].y
	    +  (val[j].y *segfil[j]) * we[ons+k].x;            
        }
    } 
}
...
static __global__ void  dosens(cufftDoubleComplex *val,
                                    cufftDoubleComplex *tmp2, double *segfil,
                                     cufftDoubleComplex *_ipk_we, int *_the_index,
									 int numP, int numK)
{     
    ...
    if (k < numK)
    {
        ...
        { 
			...
			/*
            val[k].x += (tmp2[idx].x*_ipk_we[numP*k + p].x 
				+ tmp2[idx].y*_ipk_we[numP*k + p].y) * segfil[k];
            val[k].y += (- tmp2[idx].x*_ipk_we[numP*k + p].y 
				+ tmp2[idx].y*_ipk_we[numP*k + p].x) * segfil[k];
			*/
            val[k].x += (tmp2[idx].x*_ipk_we[numP*k + p].x 
				- tmp2[idx].y*_ipk_we[numP*k + p].y) * segfil[k];
            val[k].y += (tmp2[idx].x*_ipk_we[numP*k + p].y 
...
static __global__ void diff11(cufftDoubleComplex *D,
								...
								)
{
	...
	if(x==0)
	{
		int next_pad=(x+1)+y*w+z*w*h;
		D[pad].x = Q[next_pad].x - Q[pad].x;
		D[pad].y = Q[next_pad].y - Q[pad].y;
	}
	else
	{
		int last_pad=(x-1)+y*w+z*w*h;
		D[pad].x = Q[pad].x - Q[last_pad].x;
		D[pad].y = Q[pad].y - Q[last_pad].y;

	}
}

static __global__ void diff12(cufftDoubleComplex *D,
								cufftDoubleComplex *Q,
								int w, int h, int d
								)
{
...
static __global__ void diff21(cufftDoubleComplex *D,
								cufftDoubleComplex *Q,
								int w, int h, int d
								)
{
	...
	if(y==0)
	{
		int next_pad=x+(y+1)*w+z*w*h;
		D[pad].x = Q[next_pad].x - Q[pad].x;
		D[pad].y = Q[next_pad].y - Q[pad].y;
	}
	else
	{
		int last_pad=x+(y-1)*w+z*w*h;
		D[pad].x = Q[pad].x - Q[last_pad].x;
		D[pad].y = Q[pad].y - Q[last_pad].y;
...
static __global__ void diff22(cufftDoubleComplex *D,
								cufftDoubleComplex *Q,
								int w, int h, int d
								)
{
...
static __global__ void diff31(cufftDoubleComplex *D,
								cufftDoubleComplex *Q,
								int w, int h, int d
								)
{
	...
	if(z==0)
	{
		int next_pad=x+y*w+(z+1)*w*h;
		D[pad].x = Q[next_pad].x - Q[pad].x;
		D[pad].y = Q[next_pad].y - Q[pad].y;
	}
	else
	{
		int last_pad=x+y*w+(z-1)*w*h;
		D[pad].x = Q[pad].x - Q[last_pad].x;
		D[pad].y = Q[pad].y - Q[last_pad].y;
...
static __global__ void diff32(cufftDoubleComplex *D,
								cufftDoubleComplex *Q,
								int w, int h, int d
								)
{
...
static __global__ void  downwind(cufftDoubleComplex *_r,
                                    ...
                                    cufftDoubleComplex *_sens, 
									int w, int h, int d, 
									int w_pad, int h_pad, int d_pad)
{
    int z = threadIdx.x;
    int y = blockIdx.y;
    int x = blockIdx.x;
